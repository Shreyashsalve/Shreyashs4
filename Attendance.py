...
def take_img():
    ...
    if not l1 or not l2:
        err_screen()  # Ensure you handle error screen display
        return
    ...
    try:
        ...
        # Create TrainingImage folder if not exists
        ...
        for (x, y, w, h) in faces:
            ...
            # Save only one image
            sampleNum += 1
            image_path = os.path.join(image_folder, f"{Name}.{Enrollment}.{sampleNum}.jpg")
            cv2.imwrite(image_path, gray[y:y + h, x:x + w])
            print(f"Image {sampleNum} saved: {image_path}")
            
            break  # Exit the loop after saving one image

        # Display the captured image with face
        cv2.imshow('Frame', img)
        
        # Wait for the user to press any key to close the camera window
        cv2.waitKey(0)
        ...
        # Display success notification immediately after saving the image
        res = f"Image Saved for Enrollment: {Enrollment}, Name: {Name}"
